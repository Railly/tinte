...
>(props: UseInfiniteQueryProps<T>, supabaseClient: SupabaseClient) {
  ...
  const fetchPage = async (skip: number) => {
    ...
    if (error) {
      ...
      const deduplicatedData = ((newData || []) as TData[]).filter(
        // @ts-expect-error - id is not a property of TData
...
>(props: UseInfiniteQueryProps<T>) {
  ...
  const clerkSupabaseClient = useClerkSupabase();
  
  // Use authenticated client if signed in, otherwise use public client
  const supabaseClient = isSignedIn ? clerkSupabaseClient : supabase;
  
  const storeRef = useRef(createStore<TData, T>(props, supabaseClient));
  ...
  useEffect(() => {
    // Recreate store if props change or client changes
    if (
      ...
      storeRef.current = createStore<TData, T>(props, supabaseClient);
    }

    if (!state.hasInitialFetch && typeof window !== "undefined") {
      storeRef.current.initialize();
    }
  }, [props.tableName, props.columns, props.pageSize, state.hasInitialFetch, supabaseClient, isSignedIn]);
