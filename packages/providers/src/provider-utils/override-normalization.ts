import type { NormalizedOverrides, ProviderOverride } from "@tinte/core";
...
export function normalizeOverrides(theme: unknown): NormalizedOverrides {
  const normalized: NormalizedOverrides = {};
  const themeData = theme as ThemeData;
  const themeWithOverrides = themeData as ThemeDataWithOverrides;

  const rawTheme = themeData.rawTheme as RawThemeWithExtras | undefined;
