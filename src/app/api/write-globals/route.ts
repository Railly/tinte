...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!css || typeof css !== 'string') {
      return NextResponse.json({ error: 'Invalid CSS content' }, { status: 400 });
    }
    ...
    if (!rootMatch || !darkMatch) {
      return NextResponse.json({ error: 'CSS must contain both :root and .dark sections' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: 'globals.css updated successfully' });
    
  } catch (error) {
    console.error('Failed to write globals.css:', error);
    return NextResponse.json({ error: 'Failed to write globals.css' }, { status: 500 });
  }
}