...
const createFlexokiScale = (bg: string, fg: string, existingTokens: Record<string, string>, mode: Mode) => {
  ...
  const flexokiMapping = mode === 'light' ? {
  ...
  return {
    ...
    background_2: existingTokens.card || existingTokens.popover || flexokiMapping.background_2,
...
const extractAccents = (block: TweakcnBlock, mode: Mode) => {
  ...
  const accent_2 = distinctColors[1] || rgb({
    mode: 'oklch',
    l: mode === 'light' ? 
  ...
  const accent_3 = distinctColors[2] || block.destructive || rgb({
    mode: 'oklch', 
    l: mode === 'light' ? 
