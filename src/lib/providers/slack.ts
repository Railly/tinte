...
function generateSlackTheme(theme: TinteTheme, mode: 'light' | 'dark'): SlackTheme {
...
export class SlackProvider implements ThemeProvider<{ light: SlackTheme; dark: SlackTheme }> {
  ...
  convert(theme: TinteTheme): { light: SlackTheme; dark: SlackTheme } {
    return {
  ...
  export(theme: TinteTheme, filename?: string): ProviderOutput {
    ...
    // Slack expects exactly 10 colors in a comma-separated string
  ...
  validate(output: { light: SlackTheme; dark: SlackTheme }): boolean {
    const validateTheme = (theme: SlackTheme): boolean => {
      return !!(
        theme.column_bg &&
        theme.menu_bg &&
        theme.active_item &&
