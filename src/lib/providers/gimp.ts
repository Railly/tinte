...
function generateGIMPPalette(theme: TinteTheme, mode: 'light' | 'dark'): GIMPPalette {
  const block = theme[mode];
  const colorMapping = createPolineColorMapping(block);
  
  // Create color entries for all mapped colors
  const colors: Array<{ red: number; green: number; blue: number; name: string }> = [];
...
export class GIMPProvider implements ThemeProvider<{ light: GIMPPalette; dark: GIMPPalette }> {
  ...
  export(theme: TinteTheme, filename?: string): ProviderOutput {
    ...
    const gimpContent = [
      ...
      ...palette.colors.map(color => {
        ...
        const paddedGreen = color.green.toString().padStart(maxLengths.green, ' ');
