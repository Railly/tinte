import type { WindowsTerminalTheme } from "@/lib/providers/windows-terminal";
...
export function WindowsTerminalPreview({
  theme,
  className,
}: WindowsTerminalPreviewProps) {
        ...
        <pre className="whitespace-pre-wrap leading-relaxed">
          {terminalContent.split("\n").map((line, index) => {
            if (line.startsWith("PS C:\\")) {
              ...
              line.includes("Windows Terminal") &&
              line.includes("1.18")
            ) {
              // Version info
              return (
                <div key={index}>
                  {line
                    .split(/(Windows Terminal|1\.18\.3181\.0)/)
                    .map((part, i) => {
              ...
              // Table header
              return (
                <div key={index} style={{ color: colors.prompt }}>
                  {line}
                </div>
              );
              ...
              return (
                <div key={index}>
                  {line
                    .split(
                      /(d-r---|a----|\d+\/\d+\/\d+|\d+:\d+\s+[AP]M|\d+|Desktop|Documents|Downloads|settings\.json)/,
                    )
                    .map((part, i) => {
                      if (part === "d-r---") {
                        return (
              ...
              line.includes("91d4b5b71f1d8e0e97c4d258b9ba0b7d3b6e1c5b")
            ) {
              // Commit hash
              return (
                <div key={index} style={{ color: colors.comment }}>
              ...
              return (
                <div key={index}>
                  {line
                    .split(/(Write-Host|"[^"]*"|-ForegroundColor|Green)/)
                    .map((part, i) => {
                      if (part === "Write-Host") {
                        return (
                          <span key={i} style={{ color: colors.prompt }}>
                            {part}
                          </span>
                        );
              ...
              // Output in green
              return (
                <div key={index} style={{ color: colors.success }}>
                  {line}
                </div>
              );
